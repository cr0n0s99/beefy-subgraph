...
import { StratHarvest } from '../../generated/BIFI Maxi/Strategy';
...
export function handleHarvest(event: StratHarvest): void {
  let strategy = getOrCreateStrategy(event.address);
  let transactionId = event.transaction.to
    .toHexString()
    .concat('-')
  ...
  harvest.caller = event.transaction.from;
